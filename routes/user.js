...
else
{
  ...
  if(deleteFile){
    console.log("yes");
    rimraf(config.folder_path+the_user.folder_name, function () {
...
//update capacity
router.put("/update/capacity", function (req, res) {
...
console.log(count);
  if(count==0){
    var newUser=new User({username: req.body.username, capacity: req.body.capacity, folder_name:req.body.username+"_"+date_now});
